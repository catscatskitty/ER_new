#!/usr/bin/env python3
"""
Обучение Random Forest (акустика или акустика+фонетика)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Обучение Random Forest')
...
class RandomForestTrainer:
    def __init__(self, config_path='configs', feature_type='acoustic'):
        ...
        self.models_dir = Path(self.paths_config['paths']['models_root']) / 'random_forest'
        ...
        self.logger = setup_logger('random_forest_trainer')
    ...
    def run(self):
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / 'random_forest_confusion_matrix.png')
        plt.close()
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            plt.tight_layout()
            plt.savefig(self.plots_dir / 'random_forest_feature_importance.png')
            plt.close()
        ...
        with open(self.metrics_dir / 'random_forest_metrics.json', 'w') as f:
            json.dump(metrics, f, indent=2)
        
        self.logger.info(f"Метрики сохранены в {self.metrics_dir / 'random_forest_metrics.json'}")
...
def main():
    args = parse_args()
    trainer = RandomForestTrainer(config_path=args.config, feature_type=args.features)
    trainer.run()
