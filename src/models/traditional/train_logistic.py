#!/usr/bin/env python3
"""
Обучение логистической регрессии (акустика или акустика+фонетика)
"""
...
class LogisticTrainer:
    def __init__(self, config_path='configs', feature_type='acoustic'):
        ...
        self.models_dir = Path(self.paths_config['paths']['models_root']) / 'logistic'
    ...
    def run(self):
        ...
        with open(self.metrics_dir / 'logistic_metrics.json', 'w') as f:
            json.dump(metrics, f, indent=2)
        
        self.logger.info(f"Метрики сохранены в {self.metrics_dir / 'logistic_metrics.json'}")
...
def main():
    args = parse_args()
    trainer = LogisticTrainer(config_path=args.config, feature_type=args.features)
    trainer.run()
