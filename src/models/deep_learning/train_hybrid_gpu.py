...
Использует класс CNNLSTMHybrid из cnn_lstm_hybrid.py
"""
...
from src.models.deep_learning.cnn_lstm_hybrid import CNNLSTMHybrid
...
class HybridTrainer:
    def __init__(self, config_path='configs'):
        ...
        self.models_dir = Path(self.paths_config['paths']['models_root']) / 'hybrid_gpu'
    ...
    def run(self, feature_type='acoustic'):
        self.logger.info("=" * 60)
        self.logger.info(f"ОБУЧЕНИЕ HYBRID НА {feature_type.upper()} ПРИЗНАКАХ")
        self.logger.info("=" * 60)
        ...
        model = CNNLSTMHybrid(input_dim=input_dim).to(self.device)
        ...
        plt.title(f'Confusion Matrix - Hybrid ({feature_type})')
        plt.tight_layout()
        plt.savefig(self.plots_dir / f'hybrid_{feature_type}_cm.png')
        plt.close()
        
        metrics = {
            'model': f'Hybrid_{feature_type}',
        ...
        with open(self.metrics_dir / f'hybrid_{feature_type}_metrics.json', 'w') as f:
