...
class LSTMFeatureModel(nn.Module):
    """LSTM модель для работы с извлеченными признаками"""
    
    def __init__(self, input_dim=38, hidden_size=128, num_layers=2, 
        ...
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size * num_directions, 64),
            nn.ReLU(),
            nn.Dropout(dropout),
...
# В начале класса CNNTrainer, замените строки с загрузкой конфигов:
    ...
    def run(self, feature_type='acoustic'):
        self.logger.info("=" * 60)
        self.logger.info("ОБУЧЕНИЕ LSTM НА ПРИЗНАКАХ")
        ...
        model = LSTMFeatureModel(input_dim=input_dim, num_classes=2).to(self.device)
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / 'lstm_confusion_matrix.png')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / 'lstm_training_history.png')
        plt.close()
        ...
        with open(self.metrics_dir / 'lstm_metrics.json', 'w') as f:
            json.dump(metrics, f, indent=2)
        
        clear_gpu_memory()
        
        self.logger.info(f"\nLSTM обучена. Accuracy: {report['accuracy']:.4f}")
