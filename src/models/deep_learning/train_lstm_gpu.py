#!/usr/bin/env python3
"""
Обучение LSTM модели на извлеченных признаках
Использует класс LSTMAudioClassifier из lstm_model.py
"""
...
from src.models.deep_learning.lstm_model import LSTMAudioClassifier


def parse_args():
    parser = argparse.ArgumentParser(description='Обучение LSTM на признаках')
...
class LSTMTrainer:
    def __init__(self, config_path='configs'):
        ...
        self.models_dir = Path(self.paths_config['paths']['models_root']) / 'lstm_gpu'
        ...
        self.logger = setup_logger('lstm_gpu_trainer')
    ...
    def run(self, feature_type='acoustic'):
        self.logger.info("=" * 60)
        self.logger.info(f"ОБУЧЕНИЕ LSTM НА {feature_type.upper()} ПРИЗНАКАХ")
        self.logger.info("=" * 60)
        ...
        model = LSTMAudioClassifier(input_size=input_size).to(self.device)
        ...
        for epoch in range(self.epochs):
            ...
            if val_f1 > best_val_f1:
                best_val_f1 = val_f1
                torch.save(model.state_dict(), self.models_dir / 'best_lstm.pth')
        ...
        # Тестирование
        model.load_state_dict(torch.load(self.models_dir / 'best_lstm.pth'))
        ...
        plt.title(f'Confusion Matrix - LSTM ({feature_type})')
        plt.tight_layout()
        plt.savefig(self.plots_dir / f'lstm_{feature_type}_cm.png')
        plt.close()
        
        metrics = {
            'model': f'LSTM_{feature_type}',
        ...
        with open(self.metrics_dir / f'lstm_{feature_type}_metrics.json', 'w') as f:
