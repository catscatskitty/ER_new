#!/usr/bin/env python3
"""
Мультимодальная модель: CNN на спектрограммах + LSTM на MFCC-последовательностях
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Мультимодальная модель (CNN + LSTM)')
...
class CNNBranch(nn.Module):
    """CNN для обработки спектрограмм"""
    def __init__(self):
        super().__init__()
...
class LSTMBranch(nn.Module):
    """LSTM для обработки MFCC-последовательностей"""
...
class MultimodalModel(nn.Module):
    def __init__(self, cnn_out_dim=128, lstm_out_dim=256, num_classes=2, dropout=0.5):
        super().__init__()
    ...
    def forward(self, spec, mfcc):
        cnn_feat = self.cnn_branch(spec)      # (batch, 128)
        lstm_feat = self.lstm_branch(mfcc)    # (batch, 256)
        combined = torch.cat([cnn_feat, lstm_feat], dim=1)  # (batch, 384)
        return self.fusion(combined)


class MultimodalDataset(Dataset):
    def __init__(self, spectrograms, mfcc_seq, labels):
    ...
    def __getitem__(self, idx):
        return self.spectrograms[idx], self.mfcc_seq[idx], self.labels[idx]


def load_data(processed_root):
    """Загрузка спектрограмм и MFCC-последовательностей"""
...
class MultimodalTrainer:
    def __init__(self, config_path='configs'):
        ...
        self.models_dir = Path(self.paths_config['paths']['models_root']) / 'multimodal'
        ...
        self.logger = setup_logger('multimodal')
    ...
    def load_data(self):
        (X_spec_train, X_spec_val, X_spec_test,
         X_mfcc_train, X_mfcc_val, X_mfcc_test,
         y_train, y_val, y_test) = load_data(self.processed_root)
        return (X_spec_train, X_spec_val, X_spec_test,
                X_mfcc_train, X_mfcc_val, X_mfcc_test,
                y_train, y_val, y_test)
    ...
    def run(self):
        self.logger.info("=" * 60)
        self.logger.info("ОБУЧЕНИЕ МУЛЬТИМОДАЛЬНОЙ МОДЕЛИ (CNN + LSTM)")
        self.logger.info("=" * 60)
        ...
        self.logger.info(f"Спектрограммы train: {X_spec_train.shape}, MFCC train: {X_mfcc_train.shape}")

        class_weights = self.compute_class_weights(y_train)
        weights_tensor = torch.FloatTensor(class_weights).to(self.device)

        train_dataset = MultimodalDataset(X_spec_train, X_mfcc_train, y_train)
        val_dataset = MultimodalDataset(X_spec_val, X_mfcc_val, y_val)
        test_dataset = MultimodalDataset(X_spec_test, X_mfcc_test, y_test)
        ...
        for epoch in range(self.epochs):
            ...
            history['train_loss'].append(running_loss.avg)
            ...
            if val_acc.avg > best_val_acc:
                ...
                torch.save(model.state_dict(), self.models_dir / 'best_multimodal.pth')
        ...
        # Тестирование
        model.load_state_dict(torch.load(self.models_dir / 'best_multimodal.pth'))
        model.eval()
        all_preds, all_labels = [], []
        with torch.no_grad():
            for spec, mfcc, labels in test_loader:
                spec, mfcc, labels = spec.to(self.device), mfcc.to(self.device), labels.to(self.device)
                outputs = model(spec, mfcc)
                _, pred = torch.max(outputs, 1)
                all_preds.extend(pred.cpu().numpy())
        ...
        plt.savefig(self.plots_dir / 'multimodal_cm.png')
        plt.close()
        ...
        with open(self.metrics_dir / 'multimodal_metrics.json', 'w') as f:
...
def main():
    args = parse_args()
    trainer = MultimodalTrainer(config_path=args.config)
    trainer.run()
